...
function currentTurnStartIndex(agent) {
	const events = agent.session.events;
	for (let i = events.length - 1; i >= 0; i--) if (events[i].type === "user/message") return i + 1;
	return 0;
}
/** 本轮（最后一个 user/message 之后）是否已出现 dsh-meme 表情 markdown。 */
function hasMemeInCurrentTurn(agent) {
	const events = agent.session.events;
	for (let i = currentTurnStartIndex(agent); i < events.length; i++) {
		...
		for (const block of content) if (block.type === "text" && typeof block.text === "string" && block.text.includes(API_ROOT)) return true;
	}
	return false;
}
/** 本轮输入是否由本插件补发（防回环：补发轮若仍漏配，不再补，避免无限 steer）。 */
function isCurrentTurnBackfilled(agent) {
	const events = agent.session.events;
